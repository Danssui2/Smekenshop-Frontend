...
const Highlighted = () => {
  ...
  const loadCards = () => {
    let listCards = data
      ?.slice(indexCard - cardsamount, indexCard)
      .map(function (data) {
        return (
          <Card
            key={data.product_id}
      ...
      <div className="mx-auto max-w-screen-2xl flex flex-col items-center px-4 md:px-8">
        <div className="mb-10 md:mb-16">
          ...
          <p className="mx-auto max-w-screen-md text-center text-gray-500 md:text-lg">
            Barang Pilihan, Diupdate setiap hari!
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid place-items-center gap-4 md:gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
            {cards}
          </div>
        )}
        {indexCard <= data?.length ? (
          <button
            onClick={() => {
              loadCards();
            }}
            className="inline-block mt-6 rounded-lg bg-gray-200 px-8 py-3 text-center text-sm font-semibold text-gray-500 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-300 focus-visible:ring active:text-gray-700 md:text-base">
            Muat Lebih banyak
          </button>
        ) : null}
      </div>
